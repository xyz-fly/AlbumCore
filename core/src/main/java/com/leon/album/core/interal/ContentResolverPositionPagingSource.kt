...
/**
 * @see [androidx.room.paging.LimitOffsetPagingSource]
 */
abstract class ContentResolverPositionPagingSource<Value : Any>(
    ...
    private fun initialLoad(params: LoadParams<Int>): LoadResult<Int, Value> {
        ...
        return loadFromDb(params, tempCount)
    }
    ...
    private fun queryDatabase(
        ...
        var data = emptyList<Value>()
        var cursor: Cursor? = null
        try {
            cancellationSignal = CancellationSignal()
            val limitOffsetSortOrder = getOffsetLimit(limit, offset)
            ...
            if (e is android.os.OperationCanceledException) {
                // query() can throw a framework OperationCanceledException, if it has been canceled.
        ...
        val nextPosToLoad = offset + (data.size ?: 0)
        val nextKey =
    ...
    private fun getOffsetLimit(limit: Int, offset: Int) =
